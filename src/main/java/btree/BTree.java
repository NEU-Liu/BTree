...
import btree.indexer.BasicIndexQuery;
import btree.indexer.IndexQuery;
import btree.utils.codec.VariableByteCodec;
import btree.utils.collections.longs.LongHash.BucketEntry;
import btree.utils.collections.longs.LongHash.Cleaner;
import btree.utils.collections.longs.PurgeOptObservableLongLRUMap;
import btree.utils.io.FastMultiByteArrayOutputStream;
import btree.utils.lang.ArrayUtils;
import btree.utils.lang.Primitives;
...
public class BTree extends Paged {
    ...
    public void init(boolean bulkload) throws BTreeException {
        if (!exists()) {
            ...
            if (!created) {
                throw new IllegalStateException("create B+Tree file failed: " + _file.getAbsolutePath());
            }
        } else {
            open();
        }
    ...
    private final BTreeNode getBTreeNode(BTreeRootInfo root, long page, BTreeNode parent) throws BTreeException {
        BTreeNode node;
        synchronized (_cache) {
    ...
    private final class BTreeNode implements Comparable<BTreeNode> {
         ...
         * Set leaves linked
         */
        private void setLeavesLinked(@Nonnull final BTreeNode left, @Nonnull final BTreeNode right) throws BTreeException {
        ...
        private void promoteValue(@Nonnull final Value key, final long leftPtr, final long rightPtr) throws IOException, BTreeException {
            ...
            if (!found) {
                throw new IllegalStateException("page#" + page.getPageNum() + ", insertPoint: " + insertPoint + ", leftPtr: " + leftPtr + ", ptrs: " + Arrays.toString(ptrs));
            }
         ...
         */
        void scanLeaf(@Nonnull final IndexQuery query, @Nonnull final BTreeCallback callback, final boolean edge) {
